...
public class Adapter : AdapterBase, IBondable, IPairProcess
{
	...
	public override bool SupportsExtendedAdvertising() => _bluetoothAdapter.IsExtendedAdvertisingSupported;
	...
	public async Task<BondResult> BondAsync(IDevice device, BondingOptions options, CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			else
			{
                ...
                result = await deviceInformation.Pairing.Custom.PairAsync(requestedModes).AsTask(cancellationToken);
            }
